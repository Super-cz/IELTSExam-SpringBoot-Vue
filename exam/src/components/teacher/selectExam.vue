...
<template>
  <div class="exam">
    <el-table :data="pagination.records" border>
      ...
      <el-table-column prop="examDate" sortable label="考试日期" width="120"
                       column-key="examDate"
                       :filters="filter"
  ...
  methods: {
    ...
    getExamInfo() { //分页查询所有试卷信息
      this.$axios(`/api/exams/${this.pagination.current}/${this.pagination.size}`).then(res => {
        ...
        this.examinfo = res.data.data.records
        // console.log(this.examinfo)
        let mapVal = this.examinfo.map((element,index) => { //通过map得到 filter:[{text,value}]形式的数组对象
          let newVal = {}
          newVal.text = element.examDate
          newVal.value = element.examDate
          return newVal
        })
        let hash = []
